...
|---|------|--------|--------------|-------|
| 1.1 | Audit Complex Valuation Model — verify all 15 sheets (not 16), confirm formulas, check links | ✅ READY | 0.3 | Vault file 21 corrected: 15 sheets, "Brave Blossom (2)" does not exist, SpringbokCapital is separate sheet. Sheet name "Brave Blossom " has trailing space |
| 1.2 | **Populate "Brave Blossom Capital" tab** — capital schedule from $389M concept estimate | ✅ READY | 0.3, 1.1 | ⚠️ Explicit deliverable. Tab has 882 cells of framework (year headers, 12 items, unit costs) but unit-quantity rows are zero. Use SpringbokCapital (sheet 8, 465 cells, fully populated) as template. Source data: vault file 07 |
| 1.3 | Run NPV/IRR scenarios for Brave Blossom standalone ($1,320M base case) | ✅ READY | 0.3, 1.1 | Vary discount rate (8% base), coal price ($211.20/t realisation), capital ($389M vs $525M) |
| 1.4 | Run NPV/IRR for Combined scenario (Springbok + Brave Blossom = $1,670M) | ✅ READY | 0.3, 1.1 | Sensitivity on carbon price ($45–$200/t) |
| 1.5 | Run NPV for Springbok standalone ($279M) as baseline | ✅ READY | 0.3, 1.1 | |
| 1.6 | Model the 6 SMART closure reductions — cashflow impact & NPV effect | ✅ READY | 0.3, 1.1 | ~$162M (18%) reduction. Each SMART measure individually + combined. See vault file 20 for details |
| 1.7 | Model NPV of closure liability deferral (~$495M saving @ 7%, 20yr) | ✅ READY | 0.3, 1.1 | Capital-to-liability ratio 0.43:1 |
| 1.8 | Populate "OC Clean (SandsEnd)" + "OC Clean Capital" tabs for Brumby open cut concept valuation | ✅ READY | 0.3, 1.1 | For Section 3 alternative project valuation. Sheets have formula framework (2684 + 801 cells) but values are zero. May be partial — depends on available Brumby data |
| 1.9 | Carbon cost modelling — Safeguard Mechanism baseline, emissions trajectories, ACCU revenue | ✅ READY | 0.3, 1.1 | Baselines in sheets 7/9 per vault file 21. Scenarios: $0/t vs $45–$200/t (Accelerated Transition) |
| 1.10 | Decommissioned mine emissions model — verify NGER Method 3.32 calculations | ✅ READY | 0.3, 1.1 | 4.68M tCO₂e combined scenario over ~20 years. Sheet 15 has 987 populated cells |
| 1.11 | Build summary DCF dashboard — consolidate all scenarios into one comparison view | ✅ READY | 1.3–1.10 | For presentation use |
...
| # | Task | Status | Dependencies | Notes |
|---|------|--------|--------------|-------|
| 3.13 | Cashflow analysis & NPV impact of SMART reductions | ✅ READY | 1.6 | Workbook available — can model cashflow impact of each reduction over time |
...
| 4.3 | Brumby concept valuation — populate OC Clean + OC Clean Capital tabs | ✅ READY | 1.8 | Sheets have formula framework (2684 + 801 cells) but values are zero |
...
| 4.6 | Bronco concept valuation (if pursued) | ✅ READY | 1.8, 1.1 | No resource tonnage/quality data — 🟢 lower priority gap. Can build framework from existing template |
...
| 4.11 | Business case / financial model for each forward work program | ✅ READY | 1.1–1.8 | Brief requires "accompanying business case" for all work programs. Workbook available with template structure |
...
| 5.3 | Rec 1: Stage-gated Brave Blossom — resultant cashflow analysis | ✅ READY | 1.3, 1.7 | NPV/IRR + deferral savings from workbook available |
...
| 5.5 | Rec 2: Optimise closure liability — resultant cashflow analysis | ✅ READY | 1.6 | SMART reduction cashflow impact from workbook available |
| 5.6 | Rec 2: Optimise closure liability — implementation risks | ✅ READY | 3.14 | |
| 5.7 | Rec 3: Carbon abatement & gas monetisation — resultant cashflow analysis | ✅ READY | 1.9, 1.10 | Carbon modelling + gas revenue from workbook available |
...
| 6.7 | Visual design — charts, graphs, infographics | ✅ READY | 1.11 | Financial model outputs available from workbook. SWOT matrices can be designed now |
...
| # | Task | Status | Dependencies | Notes |
|---|------|--------|--------------|-------|
| 7.1 | Anticipated board questions — compile list | ✅ READY | — | Based on brief tips: federal approval risk, coal quality, carbon cost, $900M estimate, timeline. Expand after financial modelling |
| 7.2 | Prepare model answers for top 10 likely questions | 🟡 DRAFT-READY | 7.1 | Can draft from vault + workbook. Finalise with financial data for quantitative answers |
| 7.3 | Prepare "deep dive" backup slides (appendix) | 🟡 DRAFT-READY | 7.2 | Detailed SWOT, financial sensitivity tables, carbon modelling detail. Workbook available |
...
### Phase 1 — Immediate (all unblocked tasks, start now)
Tasks: 0.2, 0.4, 0.5, 1.1–1.11 (all financial modelling — workbooks received), 2.3, 2.4–2.15 (SWOT drafts), 3.1–3.2, 3.4–3.16 (SMART targets + cashflow), 4.1, 4.3, 4.4–4.6, 4.7–4.11, 5.1, 5.3–5.10, 6.1, 6.7, 7.1, 7.5

### Phase 2 — Assembly (once analysis complete)
...
- ✅ READY (start now): 64 tasks + 8 gaps with workarounds = 72
- 🟡 DRAFT-READY (draft now, finalise later): 16
- 🔴 BLOCKED (remaining info gaps): 3
- ✅ DONE/RESOLVED: 4

*Last updated: 2026-08-29 — workbooks received and ingested, vault file 21 corrected*
